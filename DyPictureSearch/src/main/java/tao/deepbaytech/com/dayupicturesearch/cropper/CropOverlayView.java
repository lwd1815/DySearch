...
package tao.deepbaytech.com.dayupicturesearch.cropper;
...
public class CropOverlayView extends View {
  ...
  private void drawCorners(Canvas canvas) {
    if (mBorderCornerPaint != null) {
      ...
      float w =
          cornerWidth / 2 + (mCropShape == CropImageView.CropShape.RECTANGLE ? mBorderCornerOffset
              : 0);
      ...
      // Top left
      float xxlm = (cornerOffset + mBorderCornerLength) / 4;
      Paint xxPaint = new Paint();
      //xxPaint.setColor(Color.argb(119, 0, 0, 0));
      xxPaint.setAlpha(119);
      xxPaint.setStyle(Paint.Style.FILL_AND_STROKE);
      xxPaint.setAntiAlias(true);

      Path path1 = new Path();
      path1.moveTo(rect.left - cornerOffset, rect.top - cornerOffset);
      path1.lineTo(rect.left - cornerOffset + xxlm, rect.top - cornerOffset);
      path1.lineTo(rect.left - cornerOffset, rect.top - cornerOffset + xxlm);
      path1.close();
      canvas.drawPath(path1,xxPaint);

      canvas.drawLine(rect.left - cornerOffset, rect.top - cornerExtension + abc,
          rect.left - cornerOffset, rect.top + mBorderCornerLength + abc, mBorderCornerPaint);
      canvas.drawLine(rect.left - cornerExtension + abc, rect.top - cornerOffset,
          rect.left + mBorderCornerLength + abc, rect.top - cornerOffset, mBorderCornerPaint);

      RectF rectF1 = new RectF(rect.left - cornerOffset, rect.top - cornerOffset,
          rect.left + mBorderCornerLength, rect.top + mBorderCornerLength);
      canvas.drawArc(rectF1, 180, 90, false, mBorderCornerPaint);

      //Path path1 = new Path();
      //path1.addCircle(rect.left + mBorderCornerLength,rect.top + mBorderCornerLength,mBorderCornerLength+cornerOffset,
      //    Path.Direction.CCW);
      //path1.addRect(rectF1, Path.Direction.CW);
      //Paint xxPaint = new Paint();
      //xxPaint.setColor(Color.argb(119, 0, 0, 0));
      //xxPaint.setStyle(Paint.Style.FILL);
      //path1.setFillType(Path.FillType.INVERSE_EVEN_ODD);
      //canvas.drawPath(path1,xxPaint);

      // Top right
      Path path2 = new Path();
      path2.moveTo(rect.right + cornerOffset, rect.top - cornerOffset);
      path2.lineTo(rect.right + cornerOffset - xxlm, rect.top - cornerOffset);
      path2.lineTo(rect.right + cornerOffset, rect.top - cornerOffset + xxlm);
      path2.close();
      canvas.drawPath(path2,xxPaint);

      canvas.drawLine(rect.right + cornerOffset, rect.top - cornerExtension + abc,
          rect.right + cornerOffset, rect.top + mBorderCornerLength + abc, mBorderCornerPaint);
      canvas.drawLine(rect.right + cornerExtension - abc, rect.top - cornerOffset,
          rect.right - mBorderCornerLength - abc, rect.top - cornerOffset, mBorderCornerPaint);
      RectF rectF2 = new RectF(rect.right - mBorderCornerLength, rect.top - cornerOffset,
          rect.right + cornerOffset, rect.top + mBorderCornerLength);
      canvas.drawArc(rectF2, 270, 90, false, mBorderCornerPaint);

      // Bottom left
      Path path3 = new Path();
      path3.moveTo(rect.left - cornerOffset, rect.bottom + cornerOffset);
      path3.lineTo(rect.left - cornerOffset + xxlm, rect.bottom + cornerOffset);
      path3.lineTo(rect.left - cornerOffset, rect.bottom + cornerOffset - xxlm);
      path3.close();
      canvas.drawPath(path3,xxPaint);

      canvas.drawLine(rect.left - cornerOffset, rect.bottom + cornerExtension - abc,
          rect.left - cornerOffset, rect.bottom - mBorderCornerLength - abc, mBorderCornerPaint);
      canvas.drawLine(rect.left - cornerExtension + abc, rect.bottom + cornerOffset,
          rect.left + mBorderCornerLength + abc, rect.bottom + cornerOffset, mBorderCornerPaint);
      RectF rectF3 = new RectF(rect.left - cornerOffset, rect.bottom - mBorderCornerLength,
          rect.left + mBorderCornerLength, rect.bottom + cornerOffset);
      canvas.drawArc(rectF3, 90, 90, false, mBorderCornerPaint);

      // Bottom right
      Path path4 = new Path();
      path4.moveTo(rect.right + cornerOffset, rect.bottom + cornerOffset);
      path4.lineTo(rect.right + cornerOffset - xxlm, rect.bottom + cornerOffset);
      path4.lineTo(rect.right + cornerOffset, rect.bottom + cornerOffset - xxlm);
      path4.close();
      canvas.drawPath(path4,xxPaint);

      canvas.drawLine(rect.right + cornerOffset, rect.bottom + cornerExtension - abc,
          rect.right + cornerOffset, rect.bottom - mBorderCornerLength - abc, mBorderCornerPaint);
      canvas.drawLine(rect.right + cornerExtension - abc, rect.bottom + cornerOffset,
          rect.right - mBorderCornerLength - abc, rect.bottom + cornerOffset, mBorderCornerPaint);
      RectF rectF4 = new RectF(rect.right - mBorderCornerLength, rect.bottom - mBorderCornerLength,
          rect.right + cornerOffset, rect.bottom + cornerOffset);
      canvas.drawArc(rectF4, 0, 90, false, mBorderCornerPaint);
    }
  }

  public void setXxBorderColor(boolean f) {
    if (f){
      //聚焦成功
      mBorderCornerPaint.setColor(Color.argb(255,27,129,62));
    }else {
      mBorderCornerPaint.setColor(Color.argb(255,253,99,99));
    }
    invalidate();
  }

  public void setCornerOffset(int cornerOff) {
    this.abc = cornerOff;
    //invalidate();
  }

  public void drawOnlyBg(boolean b) {
    isOnlyBg = b;
    invalidate();
  }
  ...
  /**
   * Handle move of crop window using the move handler created in { #onActionDown(float, * float)}.<br>
