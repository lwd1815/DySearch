...
public class MetaballView extends View {
    ...
    private void init() {
        ...
        for (int i = 1; i < ITEM_COUNT; i++) {
            ...
            circlePath.center = new float[]{(radius * 2 + ITEM_DIVIDER) * i+radius/4*6, radius * (1f + SCALE_RATE)};
    ...
    private void metaball(Canvas canvas, int j, int i, float v, float handle_len_rate, float maxDistance) {
        ...
        //这里的left最好和draw方法中的left对应
        ball1.left = circle1.center[0]+ circle.radius;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(resolveSizeAndState((int) (ITEM_COUNT * (radius * 2 + ITEM_DIVIDER)+radius/4*6+radius/4*3), widthMeasureSpec, 0),
